...
def main():
    ...
    # Create sliders
    # --- AUTO LAYOUT (FIXES OFF-SCREEN SLIDERS) ---
    sliders = []
    ...
    usable_width = DISPLAY_WIDTH - 2 * x_margin
    col_spacing = usable_width // cols
    ...
    slider_defs = [
        ("Interaction Strength", 0.1, 128.0, 32),
        ("Particle Drift Strength", 0.05, 0.99, 0.5),
        ("Diversity Strength", 0.0, 64.0, 64),
        ("Stability Strength", 0.0, 64.0, 2),
        ("Cluster Strength", 0.0, 64.0, 64),
        ("Cluster Size Min", 1, MAX_BIN_PARTICLES, 256),
        ("Distinct Types Min", 1, NUM_TYPES, 32),
        ("α1 (multi small-scale)", 0.5, 5.0, 2.5),
        ("K1 (multi small-scale)", 0.0, 10.0, 2.5),
        ("α2 (multi mid-scale)", 0.5, 5.0, 5),
        ("K2 (multi mid-scale)", 0.0, 10.0, 5),
        ("α3 (multi large-scale)", 0.5, 10.0, 10),
        ("K3 (multi large-scale)", 0.0, 10.0, 10),
    ]

    for i, (label, min_v, max_v, val) in enumerate(slider_defs):
        x, y = slider_pos(i)
        sliders.append(Slider(x, y, slider_width, slider_height, min_v, max_v, val, label))

    # unpack back into variables (so your code still works)
    (
    ...
    void main() {
        ...
        vec3 color = v_color * (halo * 1.3 + glow * 2.8 + core * 7.5);
